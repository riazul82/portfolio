            ...
            <div className="paginationBtns">
                <div className="pageSwitchPrev">
                    <button className="pageSwitchPrevBtn" 
                        ...
                        style={(currentPage <= 1) ? {color: '#666'} : {color: '#fff'}}
                ...
                {pages.map((num, index) => {
                    return (
                        <div key={index} className="pageSwitch">
                            <button className="pageSwitchBtn" style={(currentPage === index + 1) ? {backgroundColor: 'seagreen'} : {backgroundColor: '#222'}} onClick={() => handlePageSwitch(num)}>{num}</button>
                        </div>
                    );
                })}
                <div className="pageSwitchNext">
                    <button className="pageSwitchNextBtn"
                        ...
                        style={(currentPage >= Math.ceil(itemsLength / parseInt(itemsPerPage))) ? {color: '#666'} : {color: '#fff'}}
